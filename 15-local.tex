...
% \graphicspath{{assets/figures/15/}}
...
% \begin{figure}[H]
	% \centering
	\includestandalone[mode=image]{assets/figures/15/template}
...
\begin{minipage}{.5\linewidth}
\centering
\includestandalone[mode=image]{assets/figures/15/prop1}
\end{minipage}
...
\begin{minipage}{.5\linewidth}
\centering
\includestandalone[mode=image]{assets/figures/15/prop2}
\end{minipage}
...
\begin{minipage}{.5\linewidth}
\centering
\includestandalone[mode=image]{assets/figures/15/prop3}
\end{minipage}
...
\subsubsection{Problema del flusso massimo}
...
\begin{minipage}[t]{.45\linewidth}\centering
	\begin{figure}[H]
		\centering
		\includestandalone[mode=image]{assets/figures/15/esempioFlusso}
...
\begin{minipage}[t]{.45\linewidth}\centering
	\begin{figure}[H]
		\centering
		\includestandalone[mode=image]{assets/figures/15/esempioFlusso-maxFlow}
...
\begin{minipage}{.5\linewidth}
\centering
\includestandalone[mode=image]{assets/figures/15/prop4}
\end{minipage}
...
% \begin{figure}[H]
% \centering
\includestandalone[mode=image]{assets/figures/15/reteResidua}
...
\prototype{\Matrix{\Int} \maxFlow{\Graph G, \Node s, \Node t, \Matrix{\Int} c}}{
	...
	\(f = f_0\) \Comment*[r]{Inizializza un flusso nullo}
	\(r = c\) \Comment*[r]{Capacità iniziale}

	\BlankLine
	\tcp{Fintanto che non rimane un flusso nullo}
	\Repeat{\(g = f_0\)}{
		\(g =\) trova un flusso \(r\) tale che \(\abs{g} > 0\), altrimenti \(f_0\)\;
...
% Fare riferimento alla spiegazione grafica (\href{https://youtu.be/rxH0SEakE2Y?t=2127}{minuto 35:27}) \slide{} - \slide{} slide.

% TODO maxFlow Ford-Fulkerson
% \includestandalone{assets/algorithms/15/fordFulkerson}
...
\subsubsection{Dimostrazione complessità}
...
F(S,T) &= \sum_{\mathrlap{u \in S, v \in T}}\; f(u,v) \Arrow{\(V = T - S\)} \\
       &= \sum_{\mathrlap{u \in S, v \in \mhl{V}}}\; f(u,v) - \sum_{\mathrlap{u \in S, v \in \mhl{S}}}\; f(u,v) \Arrow{conservazione del flusso\\ e antisimmetria} \\
	   &= \underbracket[1pt]{\sum_{\mathrlap{u \in S \mhl{-\{s\}},\\ v \in V}}\; f(u,v)}_{0}%
	      + \sum_{\mathrlap{v \in V}}\; f(\mhl{s},v)%
		  + \underbracket[1pt]{\sum_{\mathrlap{u \in S,\\ v \in S}}\; f(u,v)}_{0}
\end{WithArrows}
\]

dove \(\displaystyle \sum_{\mathrlap{u \in S \mhl{-\{s\}}, v \in V}}\; f(u,v) = 0\) per la conservazione del flusso, e \(\displaystyle \sum_{\mathrlap{u \in S , v \in S}}\; f(u,v) = 0\) per antisimmetria.

\(\displaystyle \sum_{\mathrlap{v \in V}}\; f(s,v) = f\) per definizone.
