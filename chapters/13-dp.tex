...
\begin{figure}[H]\centering
    \includegraphics[width=.8\textwidth]{progrdyn}
...
\`{E} una ricorrenza lineare di ordine costante, per calcolare la sua complessità applichiamo quindi il \foreign{master theorem}:
\begin{itemize}
...
\begin{figure}[H]\centering
    ...
    \caption[]{Albero di ricorsione per \fibonacciRic. Da questa figura possiamo notare che molti sottoproblemi vengono ripetuti.}
\end{figure}
...
% \section{Hateville}
% \section{Zaino}
% \subsection{Zaino con memoization}
...
%
% \subsection{Come evitare di risolvere un problema più di una volta}
%
...
% Dopodiché l'iterazione è \foreign{bottom-up}, ovvero si parte dai casi base e si va verso problemi via via sempre più grandi.
%
...
% % TODO dividi qui
%
% Possiamo fare meglio di così riducendo lo spazio utilizzato.
%
% % TODO aggiungere tabella
% \begin{algorithm}[H]
% 	\caption{fibonacci ottimizzato nello spazio}
...
% 		& tempo & spazio \\
% 		\midrule
% 			ricorsiva & \(\Omicron(n2^n)\) & \(\Omicron(n^2)\) \\
% 		\lightrule
% 			iterativa & \(\Omicron(n^2)\) & \(\Omicron(n^2)\) \\
% 		\lightrule
% 			finale & \(\Omicron(n^2)\) & \(\Omicron(n)\) \\
% 		\bottomrule
% 	\end{tabular}
...
%
% Andiamo per passi.
% Consideriamo il vicino \(i\)-esimo. Cosa succede se non accetto la sua donazione?
% Lo scarto.
...
% \[HV(i) = \{i\} \cup HV(i-2)\]
% A questo punto come faccio a decidere quale delle due opzioni scegliere?
...
%
% % TODO \help[id=EN]{aiuto con l'italiano}
...
%
% Quindi se \(i \in HV_{s}i\) allora \(HV_{s}(i) = HV_{s}(i-1)\) (ossia la soluzione per il problema \(HV_{s}i\) è la soluzione per il problema \(HV_{s}(i-1)\)), altrimenti se \(i \not\in HV_{s}i\) allora \(HV_{s}(i) = HV_{s}(i-2) \cup \{i\}\).
%
% \begin{note}
...
%
% Indichiamo con \(\abs{HV_{s}(i)}\) l'ammontare di donazioni per la soluzione ottima \(HV_{s}(i)\).
%
% \begin{note}
% Sono due casi separati poiché sono mutualmente esclusivi: o prendiamo la donazione \(i\)-esima o non la prendiamo non ci sono altre possibilità.
% \end{note}
%
% \begin{proof}[Dimostrazione primo caso: \fbox{\(i \not\in HV_{s}(i)\)}]
% Vogliamo dimostrare che \(HV_{s}(i)\) è una soluzione ottima anche per \(HV_{p}(i-1)\).
%
% Se così non fosse esisterebbe una soluzione (migliore) \(HV_{s}^{'} (i-1)\) per il problema \(HV_{p}(i-1)\) tale che \(\abs{HV_{s}^{'} (i-1)} > \abs{HV_{s}(i)}\).
%
% Ma allora \(HV_{s}^{'} (i-1)\) sarebbe una soluzione per \(HV_{p}(i)\) (tale che \(\abs{HV_{s}^{'} (i-1)} > \abs{HV_{s}(i)}\)), che è assurdo.
% Quindi \(HV_{s}(i)\) è una soluzione ottima anche per \(HV_{p}(i-1)\).
% \end{proof}
%
% \begin{proof}[Dimostrazione secondo caso: \fbox{\(i \in HV_{s}(i)\)}]
% \(i-1 \not\in HV_{s}(i)\) (il precedente non appartiene alla soluzione ottima), altrimenti non sarebbe una soluzione ammissibile.
% Quindi, \(HV_{s}(i) - \{i\}\) è una soluzione ottima per \(HV_{p} (i-2)\).
%
% Se così non fosse, esisterebbe una soluzione \(HV_{s}^{'} (i-2)\) per il problema \(HV_{p} (i-2)\) tale che \(\abs{HV_{s}^{'} (i-2)} > \abs{HV_{s} (i) - \{i\}}\).
%
% Ma allora \(HV_{s}^{'} (i-2) \cup \{i\}\) sarebbe una soluzione per \(HV_{p}(i)\) tale che \(\abs{HV_{s}^{'} (i-2) \cup \{i\}} > \abs{HV_{s} (i)}\), il che è assurdo.
% \end{proof}
%
...
%
% Possiamo quindi scrivre una formula per calcolare la somma massima date \(i\) case:
% \begin{equation*}
% 	HV(i) =
% 	\begin{dcases}
% 		0 & i = 0 \\
...
%
% % TODO \help[id=EN]{come descrivo questo paragrafo?}
...
% 	\item dobbiamo memorizzare gli insiemi nella tabella, ma è costoso quindi lo non faremo, infatti noi andremo a costruire il valore della soluzione e non la soluzione e ci permetterà di ricostruire la soluzione a posteriori.
% \end{enumerate}
%
% \subsubsection{Tabella di programmazione dinamica}
%
% Indichiamo con \(DP[i]\) il \emph{valore} della  massima quantità di donazioni che possiamo ottenere dalle prime \(i\) case di Hateville, e con \(DP[n]\) il valore della soluzione ottima.
%
...
% Questa soluzione ha complessità \(\Theta(n)\) in quanto dobbiamo fare \(\Theta(n)\) somme da fare per ottenere il risultato.
%
% \subsection{Soluzione con linguaggi di programmazione}
%
% Vediamo un paio di implementazioni con linguaggi di programmazione.
% Notiamo che gli indici iniziano da \(1\).
...
% \vspace{1ex}
%
% A questo punto abbiamo il valore della soluzione massimale, ma non abbiamo la
% soluzione (l'insieme degli indici)!
%
...
% Utilizziamo quindi questa informazione per ricostruire la soluzione in modo ricorsivo: per ricostruire la soluzione fino ad \(i\) calcoliamo i valori fino a \(i-1\) senza aggiungere nulla se la casa non è stata selezionata, altrimenti li calcoliamo fino a \(i-2\) e aggiungiamo \(i\).
%
% \begin{algorithm}[H]
...
Quando devo risolvere un sottoproblema prima controllo se l'ho già risolto guardando nella cella corrispondente della tabella dove memorizzo i risultati, altrimenti lo calcolo al momento (\foreign{on-the-fly}) chiamando ricorsivamente i sottoproblemi e scrivedno i rispettivi risultati nella tabella.
...
\begin{algorithm}[H]
	\caption{}
	\input{knapsack-limitless-memo-max}
\end{algorithm}
