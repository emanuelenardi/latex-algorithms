...
\subsubsection{Algoritmo \enquote{Shortest edges fist}}
...
\subsubsection{Discussione sulla complessità dell'algoritmo ingordo per {\tsp}}

Il costo computazionale dell'algoritmo \enquote{Shortest edges fist} ha un costo computazionale di \(\Omicron(n^2 \log n)\), dove \(\log n\) è dovuto all'ordinamento degli archi; mentre l'algoritmo \enquote{Nearest Neighbor} ha un costo di \(\Omicron(n^2)\).
...
\paragraph{Algoritmo di ricerca locale}
