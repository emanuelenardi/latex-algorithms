...
Non dovendo più tenere traccia di quali oggetti abbiamo preso e quali no, possiamo modificare la formula ricorsiva.
\[
    DP[i][c] =
    \begin{dcases}
        0                                                   & i=0\ \Or\ c=0     \\
...
Il DNA viene rappresentato attraverso una stringadi molecole basi (Adenina, Citosina, Guanina, Timina).
...
La quale ha una complessità che non vogliamo pagare in quanto ci esistono soluzioni più efficienti.
...
\prototype{\Int \LongestCommonSubsequence{\Item{} \(T\), \Item{} \(U\)}}{
    ...
    \BlankLine
    \Item{} \(\mathit{maxsofar}\) \Assign \Nil\;
    ...
    \ForEach{\text{subsequence }\(X\)\text{ of }\(T\)}{
        \If{\(X\text{ is subsequence of }U\)}{
            ...
            \BlankLine
            \If{\(\lenght(X) > \lenght(\mathit{maxsofar})\)}{
                \(\mathit{maxsofar} = X\)\;
            }
        }
    }
