...
Usando il teorema delle ricorrenze lineari di ordine costante possiamo concludere che la complessità dell'algoritmo è \(\Theta(a^n \cdot n^\beta) = \Theta(2^n)\).
...
\`{E} possibile migliorare ulteriormente l'algoritmo riducendo lo spazio utilizzato.
...
Sotto il modello di costo logaritmico (vedi\ifsubfile~sezione \enquote{valutare la dimensione dell'input}\else~\cref{sec:valutare-dimensione-input}\fi), le tre versioni hanno le complessità mostrate nella~\cref{tab:fibonacci-versions}.
