...
\begin{enumerate}
	...
	\item Si riesce a fare meglio di così?
...
???????????????????????????????????????????????
Dato che
 quindi affermare che il problema della somma dei numeri binari ha complessità \(\Theta(n)\).
...
\begin{note}
L'algoritmo ingenuo (\foreign{na\"{\i}f}) non è sempre il migliore a meno che non sia possibile dimostrare il contrario.
\end{note}
...
\paragraph{Analisi della complessità}
Il costo di esecuzione di questo algoritmo non \emph{dipende} solo dalla dimensione del vettore, ma anche \emph{dalla distribuzione dei dati in ingresso}.
...
Dopodiché viene chiamata la funzione \merge che unisce i risultati copiando i valori già ordinati su un vettore di appoggio.
????????????????????????????????
