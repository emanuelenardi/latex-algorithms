...
\begin{table}[H]\centering
	\caption{Possibili didascalie dei caratteri}
	\begin{tabular}{@{} l *{7}{c} @{}}
		\toprule
...
\begin{algorithm}[H]
	\caption{Codifica del resto tramite la compressione di Huffman}
	\input{huffman}
\end{algorithm}
...
Un arco \((u,v)\) \emph{attraversa} il taglio se \(u \in S\) e \(v \in V-S\).
Un taglio \emph{rispetta} un insieme di archi \(A\) se nessun arco di \(A\) attraversa il taglio.
\vspace{-10pt}

\begin{definition*}[arco leggero]
Un arco che attraversa un taglio è \emph{leggero} nel taglio se il suo peso è minimo fra i pesi degli archi che attraversano un taglio.
\end{definition*}
\vspace{-10pt}
...
\begin{figure}[H]\centering
	\begin{subfigure}[b]{.5\linewidth}\centering
		...
		\caption{Arco blu sicuro}
	\end{subfigure}%
		\begin{subfigure}[b]{.5\linewidth}\centering
		\includegraphics[page=6]{mst}
	\caption{Arco blu non sicuro}
	\end{subfigure}
