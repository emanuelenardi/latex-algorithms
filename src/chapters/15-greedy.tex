...
La trasformazione in un vettore di archi non viene rappresentata nel seguente algoritmo.
...
Il tempo di esecuzione per l'algoritmo di Kruskal dipende dalla realizzazione della struttura dati Merge-Find Set ({\mfConstructor}).
Utilizziamo la versione con euristica sul rango più compressione dei cammini, le cui operazioni hanno costo ammortizzato costante (\(\Omicron(1)\)).
\begin{itemize}[topsep=0pt, itemsep=0pt]
	...
	l'ordinamento degli \(m\) archi richiede \(\Omicron(m \log m)\), ma siccome il numero degli archi è limitato superiormente da \(n^2\) posso scrivere \(\Omicron(m \log n^2)\), per le proprietà dei logaritmi \(\Omicron(2 m \log n)\), ignorando le costanti moltiplicative \(\Omicron(m \log n)\);
...
\end{itemize}
Quindi l'inizializzazione costa \(\Omicron(n)\), l'ordinamento degli archi \(\Omicron(m \log n)\) e vengono eseguite \(\Omicron(m \cdot 1)\) operazioni sulla foresta di insiemi disgiunti.
Sommando i costi ottengo \(\Omicron(n + m \log n + m) = \Omicron(m \log m) = \Omicron(m \log n^2) = \Omicron(m \log n)\).
...
% Non esiste il concetto di albero radicato nel albero di copertura di peso minimo, è solo un artefatto dell'algoritmo.
...
L'albero è memorizzato tramite un \emph{vettore dei padri}, in cui \(A\) è mantenuto implicitamente, infatti è definito \(A = \{ [v, p[v]] \mid v \in V - Q - \{ r \} \}\) (in cui \(Q\) sono gli archi non ancora raggiunti, \(\{r\}\) radice).
...
\begin{itemize}[topsep=0pt, itemsep=0pt]
	...
	l'inizializzazione costa \(\Omicron(n \log n)\);
...
\end{itemize}
Per un totale di \(\Omicron(n + n \log n + m \log n) = \Omicron(m \log n)\).
...
\begin{itemize}[topsep=0pt, itemsep=0pt]
	...
	\item[\circled{\ref{prim:ciclo-esterno}}]
	il ciclo principale viene eseguito \(n-1\) per una complessità di \(\Omicron(n)\) volte dove ogni operazione di {\heapDeleteMin} costa \(\Omicron(n)\), per un totale di \(\Omicron(n^2)\);
...
La scelta della struttura dati dipende dal numero di archi: se il grafo è \emph{sparso} (con numero di archi \(m = \Omicron(n)\) o \(m = \Omicron(n \log n)\)) conviene utilizzare l'heap binario (\(\Omicron(n \log n)\) o \(\Omicron(n \log^2 n)\)) o l'algoritmo {\prim}, mentre se il grafo è \emph{denso} o addirittura completo (con numero di archi \(m = \Theta(n^2)\)) conviene utilizzare il vettore non ordinato (\(\Omicron(n^2) \subset \Omicron(n^2 \log n)\)).
