...
\prototype{\Bool \enumeration{\Item{} S, \Int n, \Int i, \dots}}{
	\params{enumeration}{S}[vettore contenente le soluzioni parziali \(S[1][i]\)]
	\params{enumeration}{n}[il numero massimo di scelte possibili]
	\params{enumeration}{i}[indice corrente]
	\params{enumeration}{\dots}[parametri addizionali dipendenti dal problema]

	\BlankLine
	\BlankLine
	\Set \(C = \getChoices{S, n, i, \dots}\) \Comment*[h]{Determina \(C\) in funzione di \(S[1][i-1]\)}

	\params{getChoices}{C}[l'insieme dei possibili candidati per estendere la soluzione]
...
\prototype{\subSets{\Array{\Int} S, \Int n, \Int k, \Int i, \Int count}}{
	\params{subSets}{n}[numeri di elementi]
	\params{subSets}{k}[numeri di elementi considerati]
	\params{subSets}{i}[la scelta che ho già fatto]
...
\prototype{\permutations{\Set \(A\), \Int \(n\), \Item[] \(S\), \Int \(i\)}}{
	\params{permutations}{A}[insieme dalla quale prendo gli oggetti]
	\params{permutations}{n}[numero di permutazioni]
...
\begin{figure}[H]
	...
	\caption{Il gioco del sudoku}\label{fig:sudoku}
\end{figure}
