...
\epigraph{\enquote{Chi si accontenta, gode}}{Proverbio}
...
\subsection[Somma di sottoinsiemi]{Somma di sottoinsiemi {\subSetSumProblem}}
...
\newpage
\subsection{Problemi fortemente, debolmente \texorpdfstring{\NP}{\textsf{NP}}-completi}
...
\begin{table}[H]\centering
	\caption{Problemi decisionali e relative grandezze}\label{tab:problem-dimensions}
	\begin{tabular}{@{} l *{3}{l} @{}}
		\toprule
...
\subsection{Esempio di problema debolmente \texorpdfstring{\NP}{\textsf{NP}}-completo}
...
\subsection{Esempi di problemi fortemente \texorpdfstring{\NP}{\textsf{NP}}-completi}
...
\[
	d[i][j] \leqslant 2 \leqslant d[i][k] + d[k][j]\qed
\]
...
\subsubsection{Algoritmo di approssimazione per \texorpdfstring{\deltaTsp}{delta-Tsp}}
...
\subsubsection{Algoritmo per \texorpdfstring{\deltaTsp}{delta-Tsp}}
...
\begin{figure}[H]
	...
	\caption[Esempio di potatura nell'albero di scelte per un algoritmo branch-\&-bound]{Se \textsf{lb}(\(S\),\(i\)) è maggiore o uguale a \(minCost\), allora si può evitare di generare ed esplorare il sottoalbero delle scelte radicato in tal nodo, effettuando così una \textbf{potatura}.}% chktex 36
\end{figure}
...
Il limite inferiore (\foreign{lower bound}) della distanza per \enquote{uscire} da \(S[i]\) (ha un costo di \(O(n)\)):
\[
	\out  = \min_{h \notin S} \{ d[S[i], h] \}
\]
\end{minipage}\hfill
...
Ad esempio la permutazione 1-2-3-4-5 ha un costo pari a \(21\).% chktex 8
