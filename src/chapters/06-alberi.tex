...
\begin{figure}[H]\centering
	\includegraphics[width=.5\textwidth]{tree-difference}
\end{figure}

\begin{note}
	Due alberi \(T\) e \(U\) che hanno gli stessi nodi, gli stessi figli per ogni nodo e la stessa radice, sono distinti qualora un nodo \(u\) sia designato come figlio \emph{sinistro} di un nodo \(v\) in \(T\), e come figlio \emph{destro} del medesimo nodo in \(U\).
...
\subsection{Applicazioni delle visite}
...
% TODO scrivere in quali esami è stata utilizzata questa funzione

\subsection*{Visita in ordine (in-visita)}

Una possibile applicazione della visita in-ordine è quella di stampare espressioni \mbox{con operatori binari}.
...
Un'altra differenza fra i due algoritmi è che quello in profondità è un algoritmo ricorsivo, mentre questo è iterativo.
...
\begin{algorithm}[H]
	\caption{Implementazione albero \enquote{primo figlio, prossimo fratello}}%
