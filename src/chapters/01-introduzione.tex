...
Quindi il numero di confronti totali per questa soluzione del problema del minimo è \(n \cdot (n - 1) = n^2 - n\), essendo \(n^2\) dominante, questo algoritmo risulta avere una complessità di \(n^2\) (più avanti saremo più precisi).
...
\begin{enumerate}
    ...
    \item \textbf{conclusione}: al termine del ciclo \(i\) è pari a \(n+1\), quindi la variabile \(min\) contiene il minimo parziale fra gli elementi compresi fra \(1\) e \(i-1\), ma visto che \(i\) vale \(n+1\), allora conterrà gli elementi il minimo fra gli elementi compresi fra \(n+1-1 = n\).
\end{enumerate}
