...
\prototype{\prim{\Graph \(G\), \Node \(r\), \Array{\Int} \(p\)}}{
	\params{r}[nodo dalla quale parto, la radice]
	\params{p}[vettore dei padri]
	...
	\BlankLine
		...
		\ForEach{\(v \in G.\adj(u)\)}{
			...
			\If{\(\Pos[v] \Neq \Nil\) \And \(\weight{u,v} < \Pos[v].\priority\)}{
				\params{\Pos[v] \Neq \Nil}[è già stato visitato]
				\params{\weight{u,v} < \Pos[v].\priority}[commenta]

				\BlankLine
				\BlankLine
				\(Q.\heapDecrease{\(\Pos[v]\), \(\weight{u,v}\)}\) \tcp{commenta}
				\(p[v] \Assign u\) \tcp{commenta}
			}
		}
	}
}
