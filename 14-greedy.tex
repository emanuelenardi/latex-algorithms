...
\begin{document}
\section{Greedy}
\begin{fquote}[Gordon Gekko][Wall Street]%
...
\begin{note}
Non è difficile scrivere gli algoritmi, la parte difficile è dimostrare che restituiscano la soluzione ottimale.
\end{note}

Molti dei problemi che vedremo li abbiamo già visti ma sono casi particolari.
...
\subsection{Insieme indipendente di intervalli}

% TODO: link a insieme indipendente di intervalli pesati
Abbiamo già risolto con la programmazione dinamica questo problema ma il fatto che tutti i pesi siano pari a 1 porta ad una semplificazione del problema.
...
\begin{figure}[bt]
	\centering
	\includestandalone{assets/figures/14/gantt_greedy}
	\caption[Una possibile soluzione ]{Una delle possibili soluzioni ingorde al problema dell'insieme indipendente di intervalli. Quali altre possibili soluzioni di cardinalità massima sono presenti? Nota come tutti gli intervalli hanno lo stesso peso.}%
	\label{fig:gantt-solution-greedy}
\end{figure}

\begin{note}
Per questo particolare problema non esiste un insieme di cardinalità 5.
\end{note}
...
\begin{enumerate}
	...
	\item \emph{dimostrare} che la scelta presa porta alla soluzione ottima;
	\item \emph{scrivere} un algoritmo ricorsivo (spesso sono iterativi) che effettua sempre la scelta ingorda.
\end{enumerate}

\subsubsection{Individuazione sottostruttura ottima}
...
Se esistesse una soluzione migliore al problema \(S[i,j]\), diciamo \(A'[i,k]\) e la sostituissi ad \(A[i,k]\) ottenerrei una soluzione migliore anche al mio problema originale, ma questo non è possibile poiché se ottenessi una soluzione migliore allora \(A[i,k]\) non sarebbe una dimostrazione ottima, il che è assurdo.
\end{proof}

\subsubsection{Definizione ricorsiva del costo della soluzione}
...
\begin{equation*}
	...
	\begin{dcases}
		0 & S[i,j] = \emptyset \\
		\max_{k \in S[i,j]} \{ \underbracket[1pt]{DP[i,k]}_{\text{prima di}\ k} + \underbracket[1pt]{DP[k,j]}_{\text{dopo di}\ k} + 1 \} & \textrm{altrimenti} \\
	\end{dcases}
\end{equation*}
...
Se l'insieme di intervalli dati in input è vuoto (\(S[i,j] = \emptyset\)), allora la dimensione dell'insieme è pari a \(0\) (caso base).
Altrimenti, se l'insieme di intervalli di partenza non è vuoto, consideriamo l'insieme di cardinalità massima.
...
\subsubsection{Verso una soluzione ingorda}

La definizione precedente ci permette di scrivere un algoritmo basato su programmazione dinamica o su memoization di complessità \(\Omicron(n^3)\): bisogna necessariamente risolvere tutti i problemi con \(i<j\), con costo \(\Omicron(n)\) nel casso pessimo.
...
Questa complessità era data dall'ordinamento del vettore che avveniva prima che i dati venissero processati indipendentemente se i dati all'interno del vettore fossero già ordinati o meno.
...
\subsubsection{Dimostrazione che è una soluzione ottima}

\begin{proof}
Fai riferimento \href{https://youtu.be/pt53LdiwN7s?t=1381}{alla spiegazione} (importante).
\end{proof}
...
\subsubsection{Scrittura dell'algoritmo}

\includestandalone{assets/algorithms/14/intervals-greedy}
...
\subsubsection{Algoritmo basato su programmazione dinamica}

\includestandalone{assets/algorithms/14/resto-dp}

\paragraph{Complessità}
\(\Omicron(nR)\) dato dai cicli innestati.
Una soluzione dipendente dal resto da dare (\(R\)) non è ottimale, si può fare meglio di così.
...
Seleziona la moneta \(j\) \textcolor{darker}{più grande} tale per cui \(t[j] \leqslant R\), e poi risolvere il problema \(S(R - t[j])\).

\subsubsection{Scrittura dell'algoritmo}

\includestandalone{assets/algorithms/14/resto-greedy}
...
\noindent
Se dimostriamo che la somma delle monete di taglio inferiore a \(k\) è minore del valore che taglio \(k\)-esimo (\(\forall k: m_k < t[k]\)), allora la soluzione (ottima) è proprio quella calcolata dall'algoritmo.
...
\begin{note}
Fare riferimento \href{https://youtu.be/pt53LdiwN7s?t=2779}{alla spiegazione}.
\end{note}

\subsection*{Approccio ingordo}
...
Fai riferimento \href{https://youtu.be/pt53LdiwN7s?t=3626}{alla spiegazione grafica}.

\subsection{Zaino frazionario}

Riproponiamo un problema visto in precedenza --- lo zaino --- ma stavolta anzichè avere la limitazione di poter prenderere o non prenderlo un oggetto (zaino \texttt{0/1}), possiamo prenderne anche frazioni di esso (zaino \emph{reale}).
...
\includestandalone{assets/algorithms/14/knapsack-greedy}
...
\begin{enumerate}
	...
	\item Otteniamo così una soluzione \(x'\) di profitto uguale o superiore, visto che il profitto specifico dell'oggetto \(1\) è massimo.
\end{enumerate}

\subsection{Compressione di Huffman}

Per rappresentare in modo efficiente i dati (minimizzare lo spazio su disco occupato, minimizzare il tempo di trasferimento su disco\dots ) abbiamo bisogno di adottare una qualche tecnica compressione.
Una fra le tante possibili è la \emph{codifica dei caratteri}.

\begin{note}
La compressione dei caratteri ha una complessità dipendente in percentuale dalle dimensioni del file di origine.
\end{note}

\begin{note}
In un codice non deve essere mai \underline{prefisso} di un altro codice, in quanto è una condizione necessaria per distinguerli.
\end{note}
...
E' possibile ottimizzare l'albero comprimendo i cammini per i caratteri \texttt{d} e \texttt{e} (come puoi vedere nella figura a destra).
Modificando di conseguenza anche la codifica dei caratteri ed ottenendo una codifica ottimizzata.

\begin{minipage}[t]{.5\linewidth}
\begin{figure}[H]
	\centering
	\includestandalone{assets/figures/14/treenode}
\end{figure}
\centering
{\ttfamily
...
\begin{figure}[H]
	\centering
	\includestandalone{assets/figures/14/treenode-compressed}
\end{figure}
\centering
{\ttfamily
\begin{tabular}{|c|c|c|c|c|}\hline
...
\subsubsection{Principio dell'algoritmo di Huffman}
Vogliamo minimizzare la lunghezza dei caratteri che compaiono più frequentemente, assegnando quindi ai caratteri con la frequenza minore i codici corrispondenti ai percorsi più lunghi all'interno dell'albero.
...
\includestandalone{assets/algorithms/14/huffman}

Siccome ogni volta devo estrarre i due elementi più piccoli faccio affidamento a un {\minHeapConstructor}.
All'inizio inserisco tutte le lettere con la priorità data dalla loro frequenza e con un nodo contenente sia la frequenza che la lettera associata.
...
\subsection{Albero di copertura minimi}
...
Questo problema prende vari nomi, come albero di copertura minimo o albero di connessione minimo.
In inglese questo problema prende il nome di \emph{Minimum Spanning Tree}.

\paragraph*{Risorse}

\begin{otherlanguage}{english}
If you are curious about what the cookie monster thing is, then watch the \href{https://www.youtube.com/watch?v=tKwnms5iRBU}{MIT lecture on Minimum Spanning Tree} of Professor Erik Demaine.
\end{otherlanguage}
...
\begin{theorem}
Un arco \([u,v]\) è detto \emph{sicuro per \(A\)} se \(A \cup \{[u,v]\}\) è ancora un sottoinsieme di qualche albero di connessione minimo.
\end{theorem}
...
\prototype{\Set \mstGenerico{\Graph G, \Array{\Int} w}}{
	...
	\Set \(A = \emptyset\) \Comment*[h]{parto da un insieme vuoto}\;
	\BlankLine
	\While{\(A\) non forma un albero di copertura}{
		trova un arco sicuro \([u,v]\)\;
		\(A = A \cup \{[u,v]\}\) \Comment*[h]{lo aggiungo all'albero}\;
	}
	\Return \(A\)\;
}
...
Si individua quindi un arco sicuro scegliendo un arco \emph{di peso minimo} fra tutti gli archi che connettono due distinti alberi (distinti da componenti connesse) della foresta.
...
L'input non è un grafo \(G\), ma un vettore di archi (\(\textsc{edge}[\,]\)) perché ho bisogno di ordinare gli archi in base la peso.
La rappresentazione dei grafi (per liste di adiacenza) non permette di ordinare gli archi in base al peso.
La trasformazione non viene rappresentata nel seguente algortmo.

\includestandalone{assets/algorithms/14/kruskal}

\paragraph{Esempio}
% TODO
Fai riferimento alla \href{}{spiegazione grafica}.
...
\circled{\ref{kruskal:init}}
L’inizializzazione richiede \(\Omicron(n)\) in quanto devo creare tutti gli alberi separati.
\circled{\ref{kruskal:order}}
...
\paragraph{Idea}
A differenza dell'algortmo di Kruskal che formava molti alberi (rappresentati da insiemi disgiunti) e successivamente li univa l'algoritmo di Prim procede mantenendo in \(A\) un singolo albero.

\begin{note}
Durante l'esecuzione dell'algoritmo esiste un solo albero che rappresenta un sottoinsieme di un albero di copertura minimo del grafo totale.
\end{note}
...
\paragraph{Implementazione}
Abbiamo bisogno di una struttura dati per i nodi non ancora nell'albero.
Durante l'esecuzione, i vertici non ancora nell'albero si trovano in una coda con min-priorità \(Q\) ordinata in base alla seguente definizione di priorità: \enquote{La priorità del nodo \(v\) è il peso minimo di un arco che collega \(v\) ad un vertice nell'albero, o \(+\infty\) se tale arco non esiste}.
...
\includestandalone{assets/algorithms/14/prim}

\paragraph{Esempio}
Fai riferimento alla \href{https://www.youtube.com/watch?v=VocW2D-Z0hk&feature=youtu.be&t=3247}{spiegazione grafica}.

\begin{figure}[H]\centering
	\includestandalone{assets/figures/14/mst}
\end{figure}
...
\circled{\ref{prim:init}}
l'inizializzazione costerebbe \(\Omicron(m \log n)\).
\circled{\ref{prim:ciclo-esterno}}
...
\circled{\ref{prim:ciclo-interno}}
...
\circled{\ref{prim:init}}
l'inizializzazione costerebbe \(\Omicron(n)\).
\circled{\ref{prim:ciclo-esterno}}
...
Quindi se il grafo è \emph{sparso} conviene utilizzare l'heap binario (\(\Omicron(m \log n)\)), mentre se il grafo è \emph{denso} (o addirittura completo) conviene utilizzare il vettore non ordinato.
