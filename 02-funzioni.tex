...
\subsubsection{regola generale \(\uparrow\)}

\exercise{\(f(n) = a_k n^k + a_{k-1} n^{k-1} + \ldots a_1 n + a_0, a_k >0 \Rightarrow f(n) = \Theta(n^k)\)}
...
f(n) &= a_k n^k + a_{k-1} n^{k-1} + \ldots + a_1 n + a_0 \Arrow{\(a_k > 0\) per def., rendo gli altri positivi}\\
	 ...
	 &\leqslant a_k n^k + \abs{a_{k-1}} n^k +  \ldots  + \abs{a_1} n^k + \abs{a_0} n^k \Arrow{raccolgo per \(n^k\)}\\
	 &= (a_k + \abs{a_{k-1}}  +  \ldots  + \abs{a_1} + \abs{a_0})\, n^k \Arrow{esiste\dots} \\
...
\iff
n > \frac{\abs{a_{k-1}} + \ldots + \abs{a_0}}{a_k} = m
\]
...
\bigskip
Ad esempio \(17n^3 - 47n^2 + 123n + 17 \Rightarrow \Theta(n^3)\).
Oppure \(2n^3 + 7 = \Theta(n^2)\).

\subsubsection{Funzioni di costo particolari}

\bigskip
La complessità di \(f(n) = 5\) è pari a \(\Theta(1)\), questa classe di complessità appartiene a quegli algoritmi che sono così ben congegnati che indipendentemente dalla dimensione dell'input ci mettono un tempo costante a risolvere il problema.
...
\begin{WithArrows}
	...
	&\Leftrightarrow g(n) \leqslant \bs{\frac{1}{c}}\: f(n), \forall n \geqslant m %
...
\begin{definition}[Eiminazione delle costanti]
\begin{align*}
...
\(
\begin{WithArrows}
f(n) = \Omicron(g(n)) &\Leftrightarrow f(n) \leqslant cg(n), \forall n \geqslant m \Arrow{Introduco una costante \(a\)}\\
	  &\Leftrightarrow f(n) \leqslant acg(n), \forall n \geqslant m, \forall a \geqslant 0 \Arrow{raccolgo \(ac\) sotto un'unica costante \(c'\)}\\
...
\begin{definition}[Sommatoria (sequenza di algoritmi)]
	\begin{align*}
	&
	\begin{dcases}
	f_1(n) = \Omicron(g_1(n)) \\
	...
	f_2(n) = \Omega(g_2(n)) \\
	\end{dcases}
	\Rightarrow f_1(n) + f_2(n) = \Omega(\minFunction(g_1(n),g_2(n)))\\
	\end{align*}
\end{definition}
...
f_1 (n) + f_2 (n) \leqslant \maxFunction\{c_1, c_2\}(2 \cdot \maxFunction(g_1(n), g_2(n)) &\Rightarrow \Arrow{pre definizione}\\
...
Mentre se ho un algoritmo lineare ed un algoritmo quadratico e la complessità risultante è una combinazione delle due, la complessità totale sarà quindi quadratica.

\begin{definition}[Cicli annidati]
Se \(f_1(n)\) viene ripetuto \(f_2(n)\) volte, allora \(f_1(n) \cdot f_2(n)\) è limitato superiormente dalla complessità maggiore fra i due e limitato inferiormente dalla complessità minore fra i due.
	\begin{align*}
	&
	\begin{dcases}
	f_1(n) = \Omicron(g_1(n)) \\
	f_2(n) = \Omicron(g_2(n)) \\
	\end{dcases}
	\Rightarrow f_1(n) \cdot f_2(n) = \Omicron(\maxFunction(g_1(n) \cdot g_2(n)))\\
	&
	\begin{dcases}
	f_1(n) = \Omega(g_1(n)) \\
	f_2(n) = \Omega(g_2(n)) \\
	\end{dcases}
	\Rightarrow f_1(n) \cdot f_2(n) = \Omega(\minFunction(g_1(n) \cdot g_2(n)))\\
	\end{align*}
\end{definition}
...
\(\begin{WithArrows}
	...
	\Arrow{elimino i quantificatori} \\
...
\subsubsection*{Altre funzioni di costo}

Vogliamo provare che \fbox{\(\log n = \Omicron(n)\)}

\medskip
Dimostriamo per induzione che \(\exists c > 0, \exists m \geqslant 0\,\colon \alert{\log n \leqslant cn}, \forall n \geqslant m\).
...
\subsubsection*{Classificazione delle funzioni}
...
Nell'analisi per livelli (o dell'albero di ricorsione) andiamo sostanzialmente a \enquote{srotolare} la ricorrenza in un albero, i cui nodi rappresentano i casti ai vari livelli della ricorsione.

\subsubsection{Esempi di analisi per livelli}
...
\'{E} possibile risolvere questa ricorrenza nel seguente modo:
\[\begin{WithArrows}
T(n) &= b + T(\frac{n}{2}) \Arrow{\(T\left(\frac{n}{2} \cdot \frac{1}{2} \right) + b\)}\\
	 ...
	 &= \cdots \\
	 &= \underbrace{b + b + \ldots + b }_{\log n}+ T(1) \Arrow{semplifico}\\
...
\subsubsection*{Analisi per livelli del primo tentativo moltiplicazione Karatsuba}
...
T(n) &= n + 4T(\nicefrac{n}{2}) \Arrow{\(4T\left(\frac{n}{2} \cdot \frac{1}{2} \right) + \frac{n}{2}\)}\\
	 ...
	 &= \cdots \\
	 &= n + 2n + 4n + 8 n + \cdots + 2^{\log n-1}n + 4^{\log n} T(1) \Arrow{raccolgo}\\
...
T(n) &= n \sum_{j=0}^{\log n-1} 2^j + 4^{\log n} \Arrow{Applico \(\forall x \neq 1: \sum_{j=0}^k x^j = \frac{x^{k+1}-1}{x-1}\),\\ dove \(k = \log n-1\)}\\[2ex]
	 ...
	 &= 2n^2 - n \Arrow{regola generale \(\uparrow\)}\\
	 &= \Theta(n^2)\\
\end{WithArrows}\]

Possiamo concludere che \(T(n) = n + 4T(\nicefrac{n}{2}) = \Theta(n^2)\).

\subsubsection*{Modifichiamo l'esempio precendente}
...
\[\begin{WithArrows}[displaystyle]
T(n) &= \sum_{i=0}^{\log n-1} 4^i \cdot \frac{n^3}{2^{3i}} + 4^{\log n} \Arrow{semplifico: \(4^i = 2^{2i}\)}\\
...
\begin{hint}
Tutte le volte che notiamo in un'equazione di riccorrenza una parte polinomiale, come ad esempio \(n^3\), possiamo dire con certezza che \(\Omega(n^3)\).
\end{hint}

Ora possiamo affermare che \(T(n) = \Theta(n^3)\).

\subsubsection*{Modifichiamo l'esempio precendente ulteriormente}

Esaminiamo la seguente equazione di ricorrenza:
\[
T(n) =
\begin{dcases}
	4T(\nicefrac{n}{2}) + n^3 & n^2 > 1 \\
	1 & n \leqslant 1
\end{dcases}
\]
...
Sommando il costo di tutti i livelli otteniamo:
\[\begin{WithArrows}[displaystyle]
T(n) &= \sum_{i=0}^{\log n-1} \frac{n^2}{2^{2i}} \cdot 4i + 4^{\log_2 n} \Arrow{semplifico: \(4^i = 2^{2i}\), \(4^{\log n} = n^{\log_2 4} = n^2\)}\\
	 ...
	 &= n^2 \log n + n^2 \Arrow{regola generale \(\uparrow\)}\\
	 &= \Theta(n^2 log n)
\end{WithArrows}\]

\subsection*{Conclusioni}

Per riassumere \(n\) ha prodotto \(n^2\), \(n^2\) ha prodotto \(n^2 \log n\) ed \(n^3\) ha prodotto \(n^3\).
Quando avremo a disposizione lo strumento \enquote{master theorem} riusciremo semplicemente guardando l'equazione di ricorrenza a capire quale complessità l'equazione di ricorrenza produce.

\subsection{Metodo di sostituzione}

Vediamo un altro meccanismo poiché il metodo precendente può non esserci di aiuto.
Il metodo di sostituzione è un metodo in cui si cerca di \enquote{indovinare} (\foreign{guess}) una soluzione in base alla propria esperienza, e si dimostra che questa soluzione è corretta tramite induzione.
...
\medskip
\textbf{Limite superiore}: Dobbiamo dimostrare che \(\exists c > 0, \exists m \geqslant 0 \,\colon \alert{T(n) \leqslant cn}, \forall n \geqslant m\)
\begin{itemize}
	\item \textbf{caso base} dimostriamo \(T(1)\):
	\[T(1) = 1 \overset{?}{\leqslant} c \cdot 1 \iff c \geqslant 1\]
	\item \textbf{ipotesi induttiva} \(\forall k < n \,\colon T(n) \leqslant ck\), ossia affermiamo che per tutti i valori più piccoli di \(n\) la dimostrazione è già stata fatta.
	...
	&\Leftrightarrow \frac{c}{2} + 1 \leqslant c\\
	&\Leftrightarrow c \geqslant 2\\
	\end{WithArrows}\]
	Abbiamo quindi provato che \(T(n) \leqslant cn\), con due diversi valori della nostra costante \(c\): nel caso base è risultata \(c \geqslant 1\), mentre nel passo induttivo è risultata pari a \(c \geqslant 1\).
...
\medskip
Al solo scopo didattico proviamo passo passo anche il limite inferiore, ossia che \fbox{\(T(n) = \Omega(n)\)}

\medskip
\textbf{Limite inferiore}: Dobbiamo dimostrare che \(\exists d > 0, \exists m \geqslant 0 \,\colon \alert{T(n) \geqslant dn}, \forall n \geqslant m\).
...
\begin{itemize}
	...
	\item \textbf{ipotesi induttiva} \(\forall k < n \,\colon T(k) \geqslant ck\), ossia affermiamo che per tutti i valori più piccoli di \(n\) la mia dimostrazione è già stata fatta.
	\item \textbf{ipotesi induttiva} dimostriamo la disequazione per \(T(n)\)
	\[\begin{WithArrows}
	T(n) &= T(\floor{\frac{n}{2}}) + n \Arrow{sost. ip. ind. con \(d = \frac{n}{2}\)}\\
		 ...
		 &\Leftrightarrow \frac{d}{2} - \frac{1}{n} + 1 \geqslant n\\
		 &\Leftrightarrow d \leqslant 2 - \frac{2}{n}\\
	\end{WithArrows}\]
...
Nota che non abbiamo fatto nemmeno ricordo all'ipotesi induttiva.

\subsubsection*{Cosa succede se si sbaglia l'intuizione}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{assets/figures/02/ennemenouno}
\end{figure}
...
Effettuiamo un tentativo e dimostriamo che \fbox{\(T(n) = \Omicron(n)\)}

\medskip
\textbf{Limite superiore}: Dobbiamo dimostrare che \(\exists c > 0, \exists m \geqslant 0 \,\colon T(n) \leqslant cn, \forall n \geqslant m\).
\begin{itemize}
	\item \textbf{caso base} lo saltiamo perchè vedremmo subito che è sbagliato.
	...
	&= (c+1)n \Arrow{obiettivo}\\
	&= (c+1)n \overset{?}{\leqslant} cn \Arrow{semplifico}\\
	&\Leftrightarrow c+1 \leqslant c
	\end{WithArrows}\]
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{assets/figures/02/tree1}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{ipotesi induttiva più stretta}: \(\exists c > 0, \exists m \geqslant 0 \,\colon T(n) \leqslant cn - b, \forall n \geqslant m\).
	...
	\[T(n) = 1 \overset{?}{\leqslant} c \cdot 1 - b \Leftrightarrow c \geqslant b + 1\]
...
\begin{itemize}
	...
	\[\begin{WithArrows}
	T(n) &= T(\floor{ \frac{n}{2} }) + T(\ceil{ \frac{n}{2} }) + 1 \Arrow{sost. ip. ind.}\\
	&\geqslant d(\floor{ \frac{n}{2} }) - b + d(\ceil{ \frac{n}{2} }) - b + 1 \Arrow{semplifichiamo}\\
	&= dn + 1 \Arrow{obiettivo}\\
	&= dn + 1 \overset{?}{\leqslant} dn\\
	\end{WithArrows}\]
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{assets/figures/02/mergesort}
\end{figure}
...
\medskip
Dobbiamo dimostrare che \(\exists c >0, \exists m \geqslant 0 \,\colon T(n) \geqslant n \log n, \forall n \geqslant m\).
...
\begin{itemize}
	...
	\[\begin{WithArrows}
	T(n) &= 2T(\floor{ \frac{n}{2} }) + n \Arrow{sost. ip. ind.}\\
		 ...
		 &\leqslant cn \log \frac{n}{2} + n \Arrow{\(\log \frac{n}{2} = \log n - \log_2 2 = \log n - 1\)}\\
		 &\leqslant cn (\log n - 1) + n \Arrow{moltiplico}\\
		 &\leqslant cn \log n - cn + n \Arrow{obiettivo}\\
	...
	\'{E} falso, ma non è un problema, non a caso si chiama notazione asintotica.
	...
	\begin{align*}
		...
		T(3) &= 2T(\floor{\frac{3}{2}}) + 2 = 5 \leqslant 1 \cdot c \cdot 3 \log 3 \Leftrightarrow c \geqslant \frac{5}{3 \log 3}\\
		T(4) &= 2T(\floor{\frac{4}{2}}) + 2 = 2T(\floor{2}) + 4
	\end{align*}
...
\subsubsection*{Ultimo esercizio}
...
\begin{itemize}
	\item \textbf{ipotesi induttiva}: \(\exists c > 0 \,\colon T(k) \geqslant ck^2, \forall k > n\)
	\item \textbf{passo induttivo} dimostriamo la disequazione per \(T(n)\):
	\[\begin{WithArrows}
	...
	\end{WithArrows}\]
	L'ultima disequazione risulta false per un termine di ordine inferiore.
	\item \textbf{ipotesi induttiva più stretta} \(\exists c > 0 \,\colon T(k) \geqslant c(k^2 - k), \forall k < n\)
	\item \textbf{passo induttivo} dimostriamo la disequazione per \(T(n)\):
	\[\begin{WithArrows}
	T(n) &= 9T\left( \floor{\frac{n}{3}} \right) + n \Arrow{sost. ip. ind. con \(k = \floor{\frac{n}{3}}\)}\\
		 ...
		 &\leqslant cn^2 - 3cn + n \overset{?}{\leqslant} cn^2 \Arrow[jump=2]{semplifico}\\
	...
	Abbiamo quindi dimostrato che \(T(n) \leqslant c(n^2 - n) \leqslant cn^2, \forall n \geqslant 0\), ossia che \(T(n) = \Omega(n^2)\)
\end{itemize}
...
\paragraph{Commento}
Affrontiamo le equazioni di ricorrenza in cui la dimensione viene divisa in \(b\) parti, dove \(b\) dev'essere almeno pari a 2; algoritmo ricorsivo dev'essere richiamato \(a\) volte, dove \(a\) è almeno 1.
...
\begin{tabular}{@{} *{5}{c} @{}}
	...
	\addlinespace
		\(1\) & \(b^{k-1}\) & \(c\,b^{(k-1)\beta}\) & \(a\)  & \(a \cdot c\, b^{k\beta}\)\\
	\addlinespace
		\(2\) & \(b^{k-2}\) & \(c\,b^{(k-2)\beta}\) & \(a^2\) & \(a^2 \cdot c\, b^{k\beta}\)\\
	\addlinespace
		\(\cdots\) & \(\cdots\) & \(\cdots\) & \(\cdots\) & \(\cdots\) \\
	\addlinespace
...
T(n) &= da^k + \sum_{i=0}^{k-1} a^i \cdot c b^{(k-i)\beta} \Arrow{moltiplico}\\
	 &= da^k + \sum_{i=0}^{k-1} a^i \cdot c b^{k\beta} + b^{-i\beta} \Arrow{porto fuori i termini\\non dipendenti da \(i\)}\\
...
\[\begin{WithArrows}[displaystyle]
T(n) &= da^k + cb^{k\beta} \sum_{i=0}^{k-1} \left( \frac{a}{b^{\beta}} \right)^{i} \Arrow{sostituisco}\\
...
Nel secondo caso se \(f(n) = n^{\beta}\) ritorniamo esattamente al secondo caso del teorema precendente, ma qui possiamo prendere in cnsiderazione funzioni più complesse.

\subsubsection*{Esercizi}

\begin{itemize}[label=\textbullet]
	\item \(T(n) = 9T(\frac{n}{3}) + n\)
	\item \(T(n) = T(\frac{2}{3} n) + n\)
	\item \(T(n) = 3T(\frac{n}{4}) + n\log n\)
	\item \(T(n) = 2T(\frac{n}{2}) + n\log n\)
\end{itemize}

\begin{theorem*}[Ricorrenze lineari di ordine costante]
Siano \uSet{a} costanti intere non negative, con \(h\) costante positiva, \(c\) e \(\beta\) costanti reali tali che \(c > 0\) e \(\beta \geqslant 0\), e sia \(T(n)\) definita dalla relazione di ricorrenza:
\[
	T(n) =
	\begin{dcases}
