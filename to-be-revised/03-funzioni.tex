...
Abbiamo concluso la prima parte delle lezioni la quale ci introduceva alle equazioni di ricorrenza, ora andremo a capire i fondamenti matematici che ci permetteranno di analizzarne una famiglia più grande.
...
Se \(f_1(n)\) viene ripetuto \(f_2(n)\) volte, allora \(f_1(n) \cdot f_2(n)\) è limitato superiormente dalla complessità maggiore fra i due e limitato inferiormente dalla complessità minore.
\begin{align*}
	&
	\begin{dcases}
	f_1(n) = \Omicron(g_1(n)) \\
...
Nell'analisi per livelli (o dell'albero di ricorsione) andiamo sostanzialmente a \enquote{srotolare} la ricorrenza in un albero, i cui nodi rappresentano i costi ai vari livelli della ricorsione.
...
Vediamo un altro meccanismo poiché il metodo precendente in alcuni casi può non esserci di aiuto.
Il metodo di sostituzione è un metodo in cui si cerca di \enquote{indovinare} (\foreign{guess}) una soluzione in base alla propria esperienza, e si dimostra che questa soluzione è corretta tramite dimostrazione per induzione.
...
\begin{itemize}
	...
	\end{WithArrows}\]
	Abbiamo quindi provato che \(T(n) \leqslant cn\), con due diversi valori della nostra costante \(c\): nel caso base è risultata \(c \geqslant 2\), mentre nel passo induttivo è risultata pari a \(c \geqslant 1\).
...
\begin{itemize}
	...
	T(n) &= T(\floor{\frac{n}{2}}) + n \Arrow{sost. ip. ind. con \(d = \frac{n}{2}\)}\\
		 ...
		 &\Rightarrow \frac{d}{2} - \frac{1}{n} + 1 \geqslant n\\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{ennemenouno}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{tree1}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{mergesort}
\end{figure}
