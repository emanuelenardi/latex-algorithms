...
In particolare ci servono a renderci conto che non stiamo dimostrando equazioni, ma disequazioni.
...
Otteniamo che \(n^2 \geqslant cn^3 \Leftrightarrow c \leqslant \frac{1}{n}\), questo significa che \(c\) diminuisce al crescere di \(n\), ovvero che non possiamo scegliere una costante \(c\) che verifichi la proprietà.
...
\begin{definition*}[\(\mathcal{O}(f(n))\) --- Limite superiore]
...
\begin{itemize}
	\item \alert{Divide}: dividi il problema in sottoproblemi di dimensione inferiore;
...
\begin{hint}
Provalo su carta!
Un algoritmo dev'essere provato per essere capito davvero!
\end{hint}
...
\paragraph{Analisi della complessità}
Il costo di esecuzione di questo algoritmo non dipende solo dalla dimensione del vettore, ma anche dalla distribuzione dei dati in ingresso.
Nel caso in cui il vettore sia \emph{già ordinato} il costo è \(\Omicron(n)\), in quanto non si entra mai nel secondo ciclo dato che la condizione risulta falsa.
...
Si svolge in tre parti:
\begin{itemize}
	\item \alert{Divide}: spezza il vettore di \(n\) elementi in 2 sottovettori di \(\frac n 2\) elementi;
